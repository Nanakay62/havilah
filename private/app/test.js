...
const App = (() => {
  ...
  function initRadarChart(dimensionsData) {
    ...
    const labels = ['Mood', 'Calm', 'Stress', 'Energy', 'Work-fit'];
      ...
      data: {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: [85, 85, 85, 85, 85], // Fixed benchmark layer
            borderDash: [4, 4],
  ...
  const commands = [
    { title: 'My Wellness (Home)', action: () => App.navigate('employee') },
    { title: 'Global Dashboard', action: () => App.navigate('landing') },
    { title: 'Notifications', action: () => App.toggleNotifications() },
    { title: 'Self-care Library', action: () => App.openResources() },
    { title: 'Binaural Beats', action: () => App.openResource('res-binaural') },
    { title: 'De-Escalation Scripts', action: () => App.openResource('res-deescalation') },
    { title: 'Chronobiology', action: () => App.openResource('res-chronobiology') },
    { title: 'Peer Walls', action: () => App.openResource('res-peerwalls') },
    { title: 'Tactical Box Breathing', action: () => App.openResource('res-breathing') },
    { title: 'Cognitive Reframing', action: () => App.openResource('res-mindfulness') },
    { title: 'Desk Mobility Reset', action: () => App.openResource('res-movement') },
    { title: 'Async Communication', action: () => App.openResource('res-boundaries') },
    { title: 'Brain-Dump Matrix', action: () => App.openResource('res-worksheets') },
    { title: 'Settings', action: () => App.openSettings() },
    { title: 'Profile Settings', action: () => App.openProfileSettings() },
    { title: 'Privacy Policy', action: () => App.openLegal('privacy') },
    { title: 'Terms of Service', action: () => App.openLegal('terms') },
    { title: 'Sign Out', action: () => App.signOut() }
  ];
    ...
    if (id === 'res-breathing') {
      ...
      htmlContent = `
        <div style="line-height: 1.6; color: var(--text-dark);">
          ...
          <button onclick="showToast('Reframe Saved', 'Great work protecting your baseline.', 'success'); App.backToResources();" style="width: 100%; padding: 12px; background: var(--purple); color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">Complete Reframe</button>
        </div>
      `;
    }
  ...
  function sortBrainDump() {
    ...
    document.getElementById('braindumpResults').innerHTML = items.map(item => 
      ...
      '<div style="display:flex; gap:8px;">' + 
        '<button onclick="App.markBrainDumpItem,
    executeCommand(this, true)" style="padding:6px 12px; background:#e2f0e8; color:var(--accent); border:none; border-radius:4px; cursor:pointer; font-size:0.85rem;">In my control</button>' + 
