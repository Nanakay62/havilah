...
const renderHeatmapReplacement = `
  async function renderHeatmap() {
    ...
    try {
      ...
      const heatmapData = json.success ? json.data : [];
      container.innerHTML = '';
      
      if (heatmapData.length === 0) {
        container.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-2);">No heatmap data available.</div>';
        return;
      }
      ...
      heatmapData.forEach((row, index) => {
        ...
        dimensions.forEach((dim, i) => {
          if (dim.key === 'name') {
            ...
            td.innerHTML = \`
              <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <span style="font-size: 0.75rem; color: #6b7280;">N=\${row.meta ? row.meta.n_size : 0}</span>
              </div>
            \`;
          ...
          if (score === null || score === undefined) {
            ...
            // Apply color mapping. 0-100 where higher is healthier.
            td.innerText = Math.round(score);
            if (score >= 80) {
              td.style.backgroundColor = '#dcfce7'; // green-100
              td.style.color = '#166534'; // green-800
            } else if (score >= 60) {
              td.style.backgroundColor = '#fef08a'; // yellow-200
              td.style.color = '#854d0e'; // yellow-800
            } else {
...
html = replaceFunction(html, 'renderHeatmap', renderHeatmapReplacement);
