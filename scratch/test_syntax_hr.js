  ...
  async function initRadarChart() {
    ...
    try {
      ...
      state.charts.radar = new Chart(ctx, {
        type: 'radar',
        data: {
          labels: ['Mood', 'Calm', 'Stress', 'Energy', 'Work-fit', 'Social', 'Purpose'],
          datasets: [
...
async function renderHeatmap() {
  try {
    ...
    mockData.forEach(dept => {
      ...
      const trackedDimensions = ['mood', 'calm', 'stress', 'energy', 'workFit', 'social', 'purpose'];

      trackedDimensions.forEach(dim => {
        let content, colorClass, tooltip;
        
        if (dept.meetsNSize === false) {
          content = '🔒';
          colorClass = 'bg-slate-300 text-slate-500 cursor-help';
          tooltip = 'title="Section Protected (N < 5 responses)"';
        } else {
          const score = dept.scores[dim] || 0;
          content = score;
          colorClass = getRiskBrickColor(score);
          tooltip = '';
        }
        ...
        rowHtml += `
          <td class="p-0 text-center align-middle">
            <div ${tooltip} class="w-full h-12 flex items-center justify-center font-bold rounded-xl text-sm transition-transform duration-150 hover:scale-[1.01] shadow-sm ${colorClass}">
              ${content}
            </div>
          </td>
        `;
  ...
  function saveProfile() {
    ...
    const _eg = document.getElementById('empGreeting'); if (_eg) { _eg.textContent = `${greeting()}, ${state.user.name.split(' ')[0]}.`; }
    closeGeneric();
