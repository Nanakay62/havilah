...
const App = (() => {
  ...
  function loadUserProfile() {
    let user = null;
    const rawUser = localStorage.getItem('wf_user');
    if (rawUser) {
    ...
    if (!user && tokenPayload) {
      user = {
        full_name: localStorage.getItem('wf_user_name') || tokenPayload.full_name || 'System Super Admin',
        email: localStorage.getItem('wf_user_email') || 'superadmin@wellframe.app',
        role: tokenPayload.isSystemSuperAdmin ? 'System Super Admin' : (tokenPayload.role || 'HR Director')
      };
    }

    if (!user) {
      user = {
        full_name: localStorage.getItem('wf_user_name') || 'Amara Okafor',
        email: localStorage.getItem('wf_user_email') || 'amara.okafor@wellframe.io',
        role: 'HR Director'
      };
    }

    let fullName = user.full_name || user.name || 'HR Admin';
    if (fullName.toLowerCase() === 'superadmin') {
      fullName = 'System Super Admin';
    }

    let role = user.role || 'HR Administrator';
    if (tokenPayload && tokenPayload.isSystemSuperAdmin) {
      role = 'System Super Admin';
      if (!user.full_name || user.full_name.toLowerCase() === 'superadmin') {
        fullName = 'System Super Admin';
      }
    }

    const email = user.email || 'superadmin@wellframe.app';
    const initials = fullName.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase() || 'SA';

    state.user = { name: fullName, email: email, role: role, avatar: initials };
    ...
    document.querySelectorAll('.dropdown-header-email').forEach(el => el.textContent = email);
  }
  ...
  function openProfileSettings() {
    ...
    const email = u.email || 'admin@wellframe.app';
  ...
  function openPulseBuilder() {
    const body = `
      ...
      <div class="form-group">
        ...
        <div class="check-group">
          <div class="check-item checked" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="phq9">PHQ-9</span><span class="check-meta">3 min</span></div>
          <div class="check-item checked" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="gad7">GAD-7</span><span class="check-meta">2 min</span></div>
          <div class="check-item" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="pss10">PSS-10</span><span class="check-meta">4 min</span></div>
          <div class="check-item" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="fas10">FAS-10</span><span class="check-meta">4 min</span></div>
          <div class="check-item" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="copsoq3_core">COPSOQ III (Core)</span><span class="check-meta">5 min</span></div>
          <div class="check-item" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="copsoq3_middle">COPSOQ III (Middle)</span><span class="check-meta">10 min</span></div>
          <div class="check-item" onclick="App.toggleCheck(this)"><div class="check-box"></div><span class="check-label" data-survey="copsoq3_long">COPSOQ III (Long)</span><span class="check-meta">15 min</span></div>
        </div>
      </div>
      <div class="form-group">
...
async function fetchActiveCycles() {
  try {
    ...
    if (!token) return;
    const res = await fetch('/api/v1/assessment-cycles/active', {
...
async function unlockAssessment(surveyType) {
  ...
  try {
    const token = localStorage.getItem('token');
    const res = await fetch('/api/v1/assessment-cycles/unlock', {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      toast('Unlock failed', err.message || 'Could not unlock assessment.', 'error');
    }
  } catch(e) {
...
async function closeAssessmentCycle(cycleId) {
  try {
    const token = localStorage.getItem('token');
    const res = await fetch('/api/v1/assessment-cycles/lock', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      toast('Close failed', err.message || 'Could not close assessment cycle.', 'error');
    }
  } catch(e) {
    toast('Network error', 'Could not reach the server.', 'error');
  }
