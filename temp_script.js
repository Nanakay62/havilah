...
const App = (() => {
  ...
  function renderHeatmap() {
    ...
    if (!container) return;
    container.innerHTML = '';
    const data = state.hrFilters.department === 'all' ? heatmapDepts : heatmapDepts.filter(d => d.dept === state.hrFilters.department);
    
    container.appendChild(el('div'));
    heatmapDims.forEach(d => container.appendChild(el('div', 'heatmap-col-label', d)));
    
    data.forEach(row => {
      container.appendChild(el('div', 'heatmap-row-label', row.dept));
      row.scores.forEach((score, i) => {
        const cell = el('div', 'heatmap-cell', score);
        cell.style.background = riskColor(score);
        cell.setAttribute('role', 'button');
        cell.setAttribute('tabindex', '0');
        cell.setAttribute('aria-label', `${row.dept} ${heatmapDims[i]}: risk score ${score}`);
        cell.title = `${row.dept} · ${heatmapDims[i]}: ${score}`;
        cell.addEventListener('click', () => {
          toast('Drill-down', `${row.dept} · ${heatmapDims[i]} — ${score < 30 ? 'Healthy range' : score < 50 ? 'Monitor — mild elevation' : score < 65 ? 'Moderate risk — action recommended' : 'Severe risk — immediate attention'}`, score < 50 ? 'info' : 'warning');
        });
